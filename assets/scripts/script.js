...
// Navbar scroll effect + active nav link scroll spy
function onScroll() {
...
function animateCounters() {
    ...
    counters.forEach(counter => {
        ...
        // Parse values like "60K+", "30K+", "3"
        const match = original.match(/^(\d+)(K?)(\+?)$/);
        if (!match) return;

        const rawNum = parseInt(match[1], 10);
        const hasK = match[2] === 'K';
        const hasPlus = match[3] === '+';
        const total = hasK ? rawNum * 1000 : rawNum;
        let current = 0;
        ...
        const timer = setInterval(() => {
            ...
            if (current >= total) {
                ...
                const display = hasK
                    ...
                    : Math.floor(current);
                counter.innerText = display + (hasPlus ? '+' : '');
            }
        }, 30);
    });
}
...
window.addEventListener('load', () => {
    ...
    const loaderBg = document.documentElement.getAttribute('data-theme') === 'light' ? '#FDFDFF' : '#0f172a';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    images.forEach(img => {
        img.addEventListener('error', function() {
            ...
            // Set placeholder content based on alt text
            if (this.alt.includes('Sumit')) {
                placeholder.innerHTML = '👨‍💻';
            } else if (this.alt.toLowerCase().includes('project') || this.alt.toLowerCase().includes('notes') || this.alt.toLowerCase().includes('archive')) {
                placeholder.innerHTML = '💻';
            } else if (this.alt.toLowerCase().includes('about')) {
                placeholder.innerHTML = '🚀';
            } else {
                placeholder.innerHTML = '📷';
            }
